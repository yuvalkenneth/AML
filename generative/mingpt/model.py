...
#                 input_vector = torch.cat((input_vector, t), dim=1)
#
...
class GPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, do_sample=False, top_k=None,
        ...
        Most likely you'll want to make sure to be in model.eval() mode of operation for this.
        """
        final_logits = torch.tensor([], device="cuda:0" if torch.cuda.is_available() else "cpu")
        probabilities = torch.tensor([], device="cuda:0" if torch.cuda.is_available() else "cpu")
        for _ in range(max_new_tokens):
            # if the sequence context is growing too long we must crop it at block_size
            if input_vector is None:
                idx_cond = idx if idx.size(1) <= self.block_size else idx[:, -self.block_size:]
                ...
                logits, _ = self(None, input_vector=input_vector)
            # pluck the logits at the final step and scale by desired temperature
            logits = logits[:, -1, :] / temperature
            # optionally crop the logits to only the top k options
            if top_k is not None:
            ...
            if do_sample:
                ...
                prob, idx_next = torch.topk(probs, k=1, dim=-1)
                probabilities = torch.cat((probabilities, prob), dim=1)
            if input_vector is None:
