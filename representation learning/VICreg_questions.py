...
def pca_tsne(enc, data):
    ...
    for image, target in data:
        ...
        tsne_plot = sns.scatterplot(x=tsne_data[:, 0], y=tsne_data[:, 1], hue=target, palette="colorblind")
        tsne_plot.set(title="TSNE")
        tsne_plot.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.show()

        pca_plot = sns.scatterplot(x=pca_data[:, 0], y=pca_data[:, 1], hue=target, palette="colorblind")
        pca_plot.set(title="PCA")
        pca_plot.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.show()


def linear_probing(train_loader, test_loader, enc, probe, dev, path_name):
    enc.eval()
    ...
    if os.path.exists(path_name):
        probe.load_state_dict(torch.load(path_name, map_location="cuda:0" if
        ...
        for epoch in range(10):
            for images, labels in train_loader:
                ...
                loss.backward()
                optimizer.step()
        # torch.save(probe.state_dict(), path_name)
    return test_linear_probe(dev, enc, probe, test_acc, test_loader)


def test_linear_probe(dev, enc, probe, test_acc, test_loader):
    probe.eval()
    with torch.no_grad():
...
def get_encoding(encoder, data_loader, device="cuda:0" if torch.cuda.is_available() else "cpu"):
    encodings, targets = [], []
...
def vic_train(enc, proj, data, optimizer, dev, test_data, mu=MU):
...
def retrieval(data, encoder, k=10, class_indices=[], original_images=None):
    nearest, farthest = get_neighbors(encoder, data, class_indices, k=k)
    plot_neighbors(original_images, nearest, farthest, k=k)
...
if __name__ == '__main__':
    ...
    trainset_loader_test_aug_no_shuffle = torch.utils.data.DataLoader(trainset_downstream,
    ...
    optim = torch.optim.Adam(
        list(no_variance_encoder.parameters()) + list(no_variance_projector.parameters()), lr=LR, betas=BETAS,
        weight_decay=WEIGHT_DECAY)

    if os.path.exists('encoder_no_variance.pth'):
        no_variance_encoder.load_state_dict(torch.load('encoder_no_variance.pth', map_location=device))
    else:
        no_variance_encoder = vic_train(no_variance_encoder, no_variance_projector, trainloader, optim,
                                        device, testloader, mu=0)
        torch.save(no_variance_encoder.state_dict(), 'encoder_no_variance.pth')
    freeze_model(no_variance_encoder)
    no_variance_probe_acc = linear_probing(train_loader=trainloader_downstream,
                                           test_loader=testloader_downstream, enc=no_variance_encoder,
                                           probe=no_variance_linear_probe, dev=device,
    ...
    if os.path.exists('no_generated_encoder.pth'):
        ...
        nearest_neighbors_set = get_nearest_neighbors(trainset_loader_test_aug_no_shuffle, 3, encoder)
    ...
    probing_acc_no_neighbors = linear_probing(train_loader=trainloader_downstream,
                                              test_loader=testloader_downstream, enc=no_generated_encoder,
                                              probe=no_generated_probe, dev=device,
                                              path_name="no_generated_probe_weights.pth")
    print('Linear Probing Accuracy: ', probing_acc_no_neighbors)

    # Q8 Retrieval Evaluation
    classes_indices = get_classes_indices(trainset_loader_test_aug_no_shuffle)
    retrieval(trainset_loader_test_aug_no_shuffle, encoder, k=5, class_indices=classes_indices,
              original_images=trainset)

    retrieval(trainset_loader_test_aug_no_shuffle, no_generated_encoder, k=5, class_indices=classes_indices,
              original_images=trainset)

    # Further evaluation
    class_images = []
    class_count = {0: 0, 7: 0}
    while any([i != 5 for i in class_count.values()]):
        index = random.randint(0, len(trainset_loader_test_aug_no_shuffle.dataset) - 1)
        image, label = trainset_loader_test_aug_no_shuffle.dataset[index]
        if class_count.get(label, None) in [0, 1, 2, 3, 4]:
            class_images.append(index)
            class_count[label] = class_count[label] + 1

    retrieval(trainset_loader_test_aug_no_shuffle, encoder, k=5, class_indices=class_images,
              original_images=trainset)

    retrieval(trainset_loader_test_aug_no_shuffle, no_generated_encoder, k=5, class_indices=class_images,
              original_images=trainset)
