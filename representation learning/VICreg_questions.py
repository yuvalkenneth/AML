...
#
#
# def get_encoding(enc, data_loader, dev="cuda" if torch.cuda.is_available() else "cpu"):
#     encodings, targets = [], []
#     for images, labels in data_loader:
#         encodings.extend(enc.encode(images.to(dev)))
...
#             classes_added += 1
#     nearest, farthest = get_neighbors(enc, data, classes_indices, k=k)
...
#
#     trainset_loader_no_aug = torch.utils.data.DataLoader(trainset_downstream, batch_size=BATCH_SIZE,
#                                                          shuffle=False, num_workers=4)
#
...
Original file is located at
    https://colab.research.google.com/drive/1AkvnfBO5trDUZdctkjq4ZVZJ1hPyoyHc
"""
#
# import os
# import random
...
#     #     codings.append(embeddings)
#     #     dataset.append(images)
#     all_embeddings = get_encoding(enc, data)[0]
#     dist_matrix = torch.cdist(all_embeddings, all_embeddings)
#     _, neighbors = torch.topk(dist_matrix, k + 1, largest=False)
#
#     # all_embeddings = torch.cat(codings, dim=0)
#     # data = torch.cat(dataset, dim=0)
#     # _, neighbors = index.search(all_embeddings.cpu(), k=k + 1)
#     return NearestNeighborsImages(data.dataset.images, neighbors[:, 1:])
#
#
# def vic_train(enc, proj, data, optimizer, dev, test_data):
#     invariance_loss, variance_loss, covariance_loss = [], [], []
...
#     #     no_variance_projector.load_state_dict(torch.load('projector_no_variance.pth', map_location=device))
#     #     freeze_model(no_variance_encoder)
#     #     no_variance_probe_acc = linear_probing(encoder_dim=ENCODER_DIM, num_classes=10, train_loader=trainloader_downstream,
#     #                                            test_loader=testloader_downstream, enc=no_variance_encoder,
...
if __name__ == '__main__':
    ...
    trainset_loader_test_aug_no_shuffle = torch.utils.data.DataLoader(trainset_downstream, batch_size=BATCH_SIZE,
                                                                      shuffle=False, num_workers=4)
    ...
    retrieval(trainset_loader_test_aug_no_shuffle, encoder, k=5)

    retrieval(trainset_loader_test_aug_no_shuffle, no_generated_encoder, k=5)
